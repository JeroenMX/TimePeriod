...
{	
	...
	public sealed class YearsTest : TestUnitBase
	{
		...
		public void YearBaseMonthTest()
		{
			...
			Assert.Equal<DateTime>( years.Start, new DateTime( year, (int)YearMonth.April, 1 ) );
		} // YearBaseMonthTest
		...
		public void SingleYearsTest()
		{
			...
			Assert.Equal<int>( years.StartYear, startYear );
			Assert.Equal<int>( years.EndYear, startYear );
		...
		public void DefaultCalendarYearsTest()
		{
			...
			Assert.Equal<int>( years.YearCount, yearCount );
			Assert.Equal<int>( years.StartYear, startYear );
			Assert.Equal<int>( years.EndYear, startYear + yearCount - 1 );
		...
		public void CustomCalendarYearsTest()
		{
			...
			Assert.Equal<int>( years.YearCount, yearCount );
			Assert.Equal<int>( years.StartYear, startYear );
			Assert.Equal<int>( years.EndYear, startYear + yearCount );

			int index = 0;
			foreach ( Year year in years.GetYears() )
			{
				Assert.Equal<DateTime>( year.Start, new DateTime( startYear + index, startMonth, 1 ) );
				index++;
			}
		...
		public void GetHalfyearsTest()
		{
			...
			foreach ( Halfyear halfyear in halfyears )
			{
				...
				Assert.Equal<int>( halfyear.Year, halfyearYear );
		...
		public void GetQuartersTest()
		{
			...
			foreach ( Quarter quarter in quarters )
			{
				...
				Assert.Equal<int>( quarter.Year, quarterYear );
		...
		public void FiscalYearsLastDayGetHalfyearsTest()
		{
			...
			Assert.Equal<DateTime>( halfyears[ 0 ].Start, new DateTime( 2006, 8, 27 ) );
			foreach ( Halfyear halfyear in halfyears )
			{
        ...
        public void FiscalYearsNearestDayGetHalfyearsTest()
		{
			...
			Assert.Equal<DateTime>( halfyears[ 0 ].Start, new DateTime( 2006, 9, 3 ) );
			foreach ( Halfyear halfyear in halfyears )
			{
				// last halfyear of a leap year
				// http://en.wikipedia.org/wiki/4-4-5_Calendar
        ...
        public void FiscalYearsLastDayGetQuartersTest()
		{
			...
			Assert.Equal<DateTime>( quarters[ 0 ].Start, new DateTime( 2006, 8, 27 ) );
			foreach ( Quarter quarter in quarters )
			{
		...
		public void FiscalYearsNearestDayGetQuartersTest()
		{
			...
			Assert.Equal<DateTime>( quarters[ 0 ].Start, new DateTime( 2006, 9, 3 ) );
			foreach ( Quarter quarter in quarters )
			{
				// last quarter of a leap year
				// http://en.wikipedia.org/wiki/4-4-5_Calendar
		...
		public void FiscalYearsNearestDayGetMonthsTest()
		{
			...
			Assert.Equal<DateTime>( months[ 0 ].Start, new DateTime( 2006, 9, 3 ) );
			for ( int i = 0; i < months.Count; i++ )
			{
				Month month = (Month)months[ i ];
