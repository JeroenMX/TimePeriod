...
namespace TimePeriod.Tests
{
	...
	public sealed class QuartersTest : TestUnitBase
	{
		...
		public void YearBaseMonthTest()
		{
			...
			Assert.Equal<DateTime>( quarters.Start, new DateTime( year, (int)YearMonth.April, 1 ) );
		} // YearBaseMonthTest
		...
		public void SingleQuartersTest()
		{
			...
			Assert.Equal<int>( quarters.StartYear, startYear );
			Assert.Equal<int>( quarters.EndYear, startYear );
		...
		public void FirstCalendarQuartersTest()
		{
			...
			Assert.Equal(YearMonth.January, quarters.YearBaseMonth);
			Assert.Equal<int>( quarters.QuarterCount, quarterCount );
			Assert.Equal( quarters.StartQuarter, startQuarter );
			Assert.Equal<int>( quarters.StartYear, startYear );
