	...
	public sealed class HalfyearTest : TestUnitBase
	{
		...
		public void DefaultCalendarTest()
		{
			...
			foreach ( YearHalfyear yearHalfyear in Enum.GetValues( typeof( YearHalfyear ) ) )
			{
				...
				Assert.Equal<int>( halfyear.BaseYear, yearStart.Year );
				Assert.Equal<DateTime>( halfyear.Start, yearStart.AddMonths( TimeSpec.MonthsPerHalfyear * offset ).Add( halfyear.Calendar.StartOffset ) );
				Assert.Equal<DateTime>( halfyear.End, yearStart.AddMonths( TimeSpec.MonthsPerHalfyear * ( offset + 1 ) ).Add( halfyear.Calendar.EndOffset ) );
			}
		} // DefaultCalendarTest
		...
		{
			int currentYear = ClockProxy.Clock.Now.Year;
			Assert.Equal<int>( new Halfyear( currentYear, YearHalfyear.First, TimeCalendar.New( YearMonth.April ) ).BaseYear, currentYear );
		...
		public void CalendarHalfyearTest()
		{
			...
			Assert.Equal<int>( h1.BaseYear, currentYear );
			Assert.Equal<DateTime>( h1.Start, new DateTime( currentYear, 1, 1 ) );
			Assert.Equal<DateTime>( h1.End, new DateTime( currentYear, 7, 1 ) );
			...
			Assert.Equal<int>( h2.BaseYear, currentYear );
			Assert.Equal<DateTime>( h2.Start, new DateTime( currentYear, 7, 1 ) );
			Assert.Equal<DateTime>( h2.End, new DateTime( currentYear + 1, 1, 1 ) );
		...
		public void DefaultHalfyearTest()
		{
			...
			Assert.Equal(YearHalfyear.First, h1.YearHalfyear);
			Assert.Equal<int>( h1.BaseYear, currentYear );
			Assert.Equal<DateTime>( h1.Start, new DateTime( currentYear, 4, 1 ) );
			Assert.Equal<DateTime>( h1.End, new DateTime( currentYear, 10, 1 ) );
			...
			Assert.Equal(YearHalfyear.Second, h2.YearHalfyear);
			Assert.Equal<int>( h2.BaseYear, currentYear );
			Assert.Equal<DateTime>( h2.Start, new DateTime( currentYear, 10, 1 ) );
			Assert.Equal<DateTime>( h2.End, new DateTime( currentYear + 1, 4, 1 ) );
		} // DefaultHalfyearTest
		...
		public void AddHalfyearsTest()
		{
			...
			Assert.Equal<int>( prevH1.BaseYear, currentYear - 1 );
			Assert.Equal<DateTime>( prevH1.Start, calendarStartDate.AddMonths( -6 ) );
			Assert.Equal<DateTime>( prevH1.End, calendarStartDate );

			Halfyear prevH2 = calendarHalfyear.AddHalfyears( -2 );
			Assert.Equal(YearHalfyear.First, prevH2.YearHalfyear);
			Assert.Equal<int>( prevH2.BaseYear, currentYear - 1 );
			Assert.Equal<DateTime>( prevH2.Start, calendarStartDate.AddMonths( -12 ) );
			Assert.Equal<DateTime>( prevH2.End, calendarStartDate.AddMonths( -6 ) );

			Halfyear prevH3 = calendarHalfyear.AddHalfyears( -3 );
			Assert.Equal(YearHalfyear.Second, prevH3.YearHalfyear);
			Assert.Equal<int>( prevH3.BaseYear, currentYear - 2 );
			Assert.Equal<DateTime>( prevH3.Start, calendarStartDate.AddMonths( -18 ) );
			Assert.Equal<DateTime>( prevH3.End, calendarStartDate.AddMonths( -12 ) );

			Halfyear futureH1 = calendarHalfyear.AddHalfyears( 1 );
			Assert.Equal(YearHalfyear.Second, futureH1.YearHalfyear);
			Assert.Equal<int>( futureH1.BaseYear, currentYear );
			Assert.Equal<DateTime>( futureH1.Start, calendarStartDate.AddMonths( 6 ) );
			Assert.Equal<DateTime>( futureH1.End, calendarStartDate.AddMonths( 12 ) );

			Halfyear futureH2 = calendarHalfyear.AddHalfyears( 2 );
			Assert.Equal(YearHalfyear.First, futureH2.YearHalfyear);
			Assert.Equal<int>( futureH2.BaseYear, currentYear + 1 );
			Assert.Equal<DateTime>( futureH2.Start, calendarStartDate.AddMonths( 12 ) );
			Assert.Equal<DateTime>( futureH2.End, calendarStartDate.AddMonths( 18 ) );

			Halfyear futureH3 = calendarHalfyear.AddHalfyears( 3 );
			Assert.Equal(YearHalfyear.Second, futureH3.YearHalfyear);
			Assert.Equal<int>( futureH3.BaseYear, currentYear + 1 );
			Assert.Equal<DateTime>( futureH3.Start, calendarStartDate.AddMonths( 18 ) );
			Assert.Equal<DateTime>( futureH3.End, calendarStartDate.AddMonths( 24 ) );
		} // AddHalfyearsTest
