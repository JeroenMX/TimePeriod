...
{	
	...
	public sealed class TimeRangeTest : TestUnitBase
	{
		...
		public void MomentTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.Start, moment );
			Assert.Equal<DateTime>( timeRange.End, moment );
		...
		public void StartEndTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.Start, start );
			Assert.Equal<DateTime>( timeRange.End, end );
			Assert.Equal<TimeSpan>( timeRange.Duration, duration );
		...
		public void StartNegativeTimeSpanTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.Start, start.Add( timeSpan ) );
			Assert.Equal<TimeSpan>( timeRange.Duration, timeSpan.Negate() );
			Assert.Equal<DateTime>( timeRange.End, start );
			Assert.False( timeRange.IsAnytime );
			Assert.False( timeRange.IsMoment );
			Assert.False( timeRange.IsReadOnly );
		...
		public void StartTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.Start, changedStart );
		} // StartTest
		...
		public void EndTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.End, changedEnd );
		} // EndTest
		...
		public void CopyTest()
		{
			...
			Assert.Equal<DateTime>( noMoveTimeRange.Start, start );
			Assert.Equal<DateTime>( noMoveTimeRange.End, end );
			Assert.Equal<TimeSpan>( noMoveTimeRange.Duration, duration );

			TimeSpan forwardOffset = new TimeSpan( 2, 30, 15 );
			ITimeRange forwardTimeRange = timeRange.Copy( forwardOffset );
			Assert.Equal<DateTime>( forwardTimeRange.Start, start.Add( forwardOffset ) );
			Assert.Equal<DateTime>( forwardTimeRange.End, end.Add( forwardOffset ) );

			TimeSpan backwardOffset = new TimeSpan( -1, 10, 30 );
			ITimeRange backwardTimeRange = timeRange.Copy( backwardOffset );
			Assert.Equal<DateTime>( backwardTimeRange.Start, start.Add( backwardOffset ) );
			Assert.Equal<DateTime>( backwardTimeRange.End, end.Add( backwardOffset ) );
		} // CopyTest
		...
		public void MoveTest()
		{
			...
			Assert.Equal<DateTime>( timeRangeMoveZero.Start, start );
			Assert.Equal<DateTime>( timeRangeMoveZero.End, end );
			Assert.Equal<TimeSpan>( timeRangeMoveZero.Duration, duration );

			TimeRange timeRangeMoveForward = new TimeRange( start, end );
			TimeSpan forwardOffset = new TimeSpan( 2, 30, 15 );
			timeRangeMoveForward.Move( forwardOffset );
			Assert.Equal<DateTime>( timeRangeMoveForward.Start, start.Add( forwardOffset ) );
			Assert.Equal<DateTime>( timeRangeMoveForward.End, end.Add( forwardOffset ) );

			TimeRange timeRangeMoveBackward = new TimeRange( start, end );
			TimeSpan backwardOffset = new TimeSpan( -1, 10, 30 );
			timeRangeMoveBackward.Move( backwardOffset );
			Assert.Equal<DateTime>( timeRangeMoveBackward.Start, start.Add( backwardOffset ) );
			Assert.Equal<DateTime>( timeRangeMoveBackward.End, end.Add( backwardOffset ) );
		} // MoveTest
		...
		public void ExpandStartToTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.Start, start.AddMinutes( -1 ) );
		} // ExpandStartToTest
		...
		public void ExpandEndToTest()
		{
			...
			Assert.Equal<DateTime>( timeRange.End, end.AddMinutes( 1 ) );
		} // ExpandEndToTest
		...
		public void GetRelationTest()
		{
			...
			// reference
			Assert.Equal<DateTime>( testData.Reference.Start, start );
			Assert.Equal<DateTime>( testData.Reference.End, end );
			...
			Assert.Equal<DateTime>( testData.StartTouching.End, start );
			...
			Assert.Equal<DateTime>( testData.InsideStartTouching.Start, start );
			...
			Assert.Equal<DateTime>( testData.EnclosingStartTouching.Start, start );
			...
			Assert.Equal<DateTime>( testData.EnclosingEndTouching.End, end );
			...
			Assert.Equal<DateTime>( testData.ExactMatch.Start, start );
			Assert.Equal<DateTime>( testData.ExactMatch.End, end );
			...
			Assert.Equal<DateTime>( testData.InsideEndTouching.End, end );
			...
			Assert.Equal<DateTime>( testData.EndTouching.Start, end );
