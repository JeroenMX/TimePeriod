...
{		
	public sealed class TimeIntervalTest : TestUnitBase
	{
		...
		public void MomentTest()
		{
			...
			Assert.Equal<DateTime>( timeInterval.Start, moment );
			Assert.Equal<DateTime>( timeInterval.End, moment );
		...
		public void StartEndIncludeTest()
		{
			...
			Assert.Equal<DateTime>( timeInterval.StartInterval, start );
			Assert.Equal<DateTime>( timeInterval.End, end );
			Assert.Equal<DateTime>( timeInterval.EndInterval, end );
			Assert.Equal<TimeSpan>( timeInterval.Duration, duration );
		...
		public void StartEndExcludeTest()
		{
			...
			Assert.NotEqual<DateTime>( timeInterval.Start, start );
			Assert.Equal<DateTime>( timeInterval.StartInterval, start );
			Assert.NotEqual<DateTime>( timeInterval.End, end );
			Assert.Equal<DateTime>( timeInterval.EndInterval, end );
			Assert.Equal<TimeSpan>( timeInterval.Duration, duration );
			Assert.False( timeInterval.IsAnytime );
			Assert.False( timeInterval.IsMoment );
		...
		public void StartIntervalIncludeTest()
		{
			...
			Assert.Equal<DateTime>( timeInterval.StartInterval, changedStart );
		...
		public void EndIntervalIncludeTest()
		{
			...
			Assert.Equal<DateTime>( timeInterval.EndInterval, changedEnd );
		} // IntervalIncludeEnd
		...
		public void MoveTest()
		{
			...
			Assert.Equal<DateTime>( timeIntervalMoveZero.Start, start );
			Assert.Equal<DateTime>( timeIntervalMoveZero.End, end );
			Assert.Equal<TimeSpan>( timeIntervalMoveZero.Duration, duration );

			TimeInterval timeIntervalMoveForward = new TimeInterval( start, end );
			TimeSpan forwardOffset = new TimeSpan( 2, 30, 15 );
			timeIntervalMoveForward.Move( forwardOffset );
			Assert.Equal<DateTime>( timeIntervalMoveForward.Start, start.Add( forwardOffset ) );
			Assert.Equal<DateTime>( timeIntervalMoveForward.End, end.Add( forwardOffset ) );

			TimeInterval timeIntervalMoveBackward = new TimeInterval( start, end );
			TimeSpan backwardOffset = new TimeSpan( -1, 10, 30 );
			timeIntervalMoveBackward.Move( backwardOffset );
			Assert.Equal<DateTime>( timeIntervalMoveBackward.Start, start.Add( backwardOffset ) );
			Assert.Equal<DateTime>( timeIntervalMoveBackward.End, end.Add( backwardOffset ) );
		} // MoveTest
