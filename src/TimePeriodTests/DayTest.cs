...
namespace TimePeriod.Tests
{
	...
	public sealed class DayTest : TestUnitBase
	{
		...
		public void DefaultCalendarTest()
		{
			...
			foreach ( YearMonth yearMonth in Enum.GetValues( typeof( YearMonth ) ) )
			{
				...
				for ( int monthDay = monthStart.Day; monthDay < monthEnd.Day; monthDay++ )
				{
					...
					Assert.Equal<int>( day.Year, yearStart.Year );
					Assert.Equal<int>( day.Month, monthStart.Month );
					Assert.Equal<int>( day.Month, monthEnd.Month );
					Assert.Equal<int>( day.DayValue, monthDay );
					Assert.Equal<DateTime>( day.Start, monthStart.AddDays( monthDay - 1 ).Add( day.Calendar.StartOffset ) );
					Assert.Equal<DateTime>( day.End, monthStart.AddDays( monthDay ).Add( day.Calendar.EndOffset ) );
				}
			}
		} // DefaultCalendarTest
		...
		public void ConstructorTest()
		{
			...
			Assert.Equal<int>( new Day( now ).Year, now.Year );
			Assert.Equal<int>( new Day( now ).Month, now.Month );
			Assert.Equal<int>( new Day( now ).DayValue, now.Day );

			Assert.Equal<int>( new Day( now.Year, now.Month, now.Day ).Year, now.Year );
			Assert.Equal<int>( new Day( now.Year, now.Month, now.Day ).Month, now.Month );
			Assert.Equal<int>( new Day( now.Year, now.Month, now.Day ).DayValue, now.Day );
		} // ConstructorTest
		...
		public void AddDaysTest()
		{
			...
			Assert.Equal<int>( day.AddDays( -1 ).Year, previousDay.Year );
			Assert.Equal<int>( day.AddDays( -1 ).Month, previousDay.Month );
			Assert.Equal<int>( day.AddDays( -1 ).DayValue, previousDay.Day );

			DateTime nextDay = nowDay.AddDays( 1 );
			Assert.Equal<int>( day.AddDays( 1 ).Year, nextDay.Year );
			Assert.Equal<int>( day.AddDays( 1 ).Month, nextDay.Month );
			Assert.Equal<int>( day.AddDays( 1 ).DayValue, nextDay.Day );
		} // AddDaysTest
