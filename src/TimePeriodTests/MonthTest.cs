	...
	public sealed class MonthTest : TestUnitBase
	{
		...
		public void DefaultCalendarTest()
		{
			...
			foreach ( YearMonth yearMonth in Enum.GetValues( typeof( YearMonth ) ) )
			{
				...
				Assert.Equal<int>( month.Year, yearStart.Year );
				Assert.Equal( month.YearMonth, yearMonth );
				Assert.Equal<DateTime>( month.Start, yearStart.AddMonths( offset ).Add( month.Calendar.StartOffset ) );
				Assert.Equal<DateTime>( month.End, yearStart.AddMonths( offset + 1 ).Add( month.Calendar.EndOffset ) );
			}
		} // DefaultCalendarTest
		...
		public void CurrentMonthTest()
		{
			...
			foreach ( CultureInfo culture in cultures )
			{
				...
				Assert.Equal<int>( month.Year, now.Year );
				Assert.Equal( month.YearMonth, (YearMonth)now.Month );
				Assert.Equal<DateTime>( month.Start, currentYearMonthStart );
				Assert.Equal<DateTime>( month.End, currentYearMonthEnd );
			}
		} // CurrentMonthTest
		...
		public void YearMonthTest()
		{
			...
			Assert.Equal<DateTime>( january.Start, new DateTime( currentYear, 1, 1 ) );
			Assert.Equal<DateTime>( january.End, new DateTime( currentYear, 2, 1 ) );
		} // YearMonthTest
		...
		public void MonthMomentTest()
		{
			...
			Assert.Equal<DateTime>( month.Start, new DateTime( 2008, 1, 1 ) );
			Assert.Equal<DateTime>( month.End, new DateTime( 2008, 2, 1 ) );

			Month previous = month.GetPreviousMonth();
			Assert.Equal(2007, previous.Year);
			Assert.Equal(YearMonth.December, previous.YearMonth);
			Assert.Equal<DateTime>( previous.Start, new DateTime( 2007, 12, 1 ) );
			Assert.Equal<DateTime>( previous.End, new DateTime( 2008, 1, 1 ) );

			Month next = month.GetNextMonth();
			Assert.Equal(2008, next.Year);
			Assert.Equal(YearMonth.February, next.YearMonth);
			Assert.Equal<DateTime>( next.Start, new DateTime( 2008, 2, 1 ) );
			Assert.Equal<DateTime>( next.End, new DateTime( 2008, 3, 1 ) );
		} // MonthMomentTest
		...
		{
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.January ).LastDayStart, new DateTime( 2001, 1, 31 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.February ).LastDayStart, new DateTime( 2001, 2, 28 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.March ).LastDayStart, new DateTime( 2001, 3, 31 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.April ).LastDayStart, new DateTime( 2001, 4, 30 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.May ).LastDayStart, new DateTime( 2001, 5, 31 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.June ).LastDayStart, new DateTime( 2001, 6, 30 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.July ).LastDayStart, new DateTime( 2001, 7, 31 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.August ).LastDayStart, new DateTime( 2001, 8, 31 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.September ).LastDayStart, new DateTime( 2001, 9, 30 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.October ).LastDayStart, new DateTime( 2001, 10, 31 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.November ).LastDayStart, new DateTime( 2001, 11, 30 ) );
			Assert.Equal<DateTime>( new Month( 2001, YearMonth.December ).LastDayStart, new DateTime( 2001, 12, 31 ) );

			Assert.Equal<DateTime>( new Month( 2000, YearMonth.February ).LastDayStart, new DateTime( 2000, 2, 29 ) );
		} // LastDayStartTest
		...
		public void AddMonthsTest()
		{
			...
			foreach ( CultureInfo culture in cultures )
			{
				...
				Assert.Equal<DateTime>( currentYearMonth.Start, currentYearMonthStart );
				Assert.Equal<DateTime>( currentYearMonth.End, currentYearMonthEnd );
				...
				Assert.Equal<DateTime>( previousYearMonth.Start, previousYearMonthStart );
				Assert.Equal<DateTime>( previousYearMonth.End, previousYearMonthEnd );
				...
				Assert.Equal<DateTime>( nextYearMonth.Start, nextYearMonthStart );
				Assert.Equal<DateTime>( nextYearMonth.End, nextYearMonthEnd );
