...
{	
	...
	public sealed class WeekTest : TestUnitBase
	{
			...
			foreach ( CultureInfo culture in cultures )
			{
				foreach ( YearWeekType yearWeekType in Enum.GetValues( typeof( YearWeekType ) ) )
				{
					...
					for ( int weekOfYear = startWeek; weekOfYear < weeksOfYear; weekOfYear++ )
					{
						...
						Assert.Equal<int>( week.Year, currentYear );
						...
						Assert.Equal<DateTime>( week.Start, weekStart.Add( week.Calendar.StartOffset ) );
						Assert.Equal<DateTime>( week.End, weekEnd.Add( week.Calendar.EndOffset ) );
					}
				}
			}
		} // DefaultCalendarTest
		...
		public void EnAuCultureTest()
		{
			...
			Assert.Equal<DateTime>( week.Start, new DateTime( 2011, 3, 28 ) );
		} // EnAuCultureTest
