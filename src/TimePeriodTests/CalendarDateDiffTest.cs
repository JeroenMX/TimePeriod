...
namespace TimePeriod.Tests
{
	...
	public sealed class CalendarDateDiffTest : TestUnitBase
	{
		...
		public void EmptyDiffTest()
		{
			...
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date, date ), TimeSpan.Zero );
		} // EmptyDiffTest
		...
		public void NoFilterTest()
		{
			...
			CalendarDateDiff calendarDateDiff = new CalendarDateDiff();
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date1, date1.AddHours( 1 ) ), new TimeSpan( 1, 0, 0 ) );
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date1, date1.AddHours( -1 ) ), new TimeSpan( -1, 0, 0 ) );
		} // NoFilterTest
		...
		public void HourFilterTest()
		{
			...
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date1, date2 ), new TimeSpan( 74, 0, 0 ) );
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date2, date1 ), new TimeSpan( -74, 0, 0 ) );
		} // HourFilterTest
		...
		public void DayHourFilterTest()
		{
			...
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date1, date2 ), new TimeSpan( 75, 0, 0 ) );
			Assert.Equal<TimeSpan>( calendarDateDiff.Difference( date2, date1 ), new TimeSpan( -75, 0, 0 ) );
		} // DayHourFilterTest
